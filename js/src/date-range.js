...
/*
  ...
  className,
  show:propShow = true,
  monthsNames = ['Jan', 'Feb', 'Mar', 'Apr', 'Mai', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Now', 'Dec'],
...
const DefaultType = {
  ...
  minDate: 'string',
  maxDate: 'string',
  onChange?: 'function', //(dateFrom:string, dateTo?:string)=>void
}
...
const DATA_KEY = 'coreui.date-range'
...
//
const SELECTOR_COMPONENT = '.c-data-range'
...
class DateRange extends BaseComponent {
  ...
  // callback (month in range of 0-11)
  ...
  // on change callback
  onChange(type:string, date:string, dateTo?:string) {
    type
          ...
          >
            <div style=${style_space}>
              <CDatePicker
                ...
                onChange=${(date:string, dateTo?:string) => {return onChange('from', date, dateTo)}}
                showNavi=${t.showNavi}
                monthsNames=${t.monthsNames}
                daysNames=${t.daysNames}
                minDate=${t.minDate}
                maxDate=${t.maxDate}
              />
            </div>
            <div style={style_separator}></div>
            <div style={style_space}>
              <CDatePicker
                ${t.range}
                ${t.helper}
                ...
                dateTo=${t.dateTo}
                focus=${t.toMinDate}
                onChange=${(date:string, dateTo?:string) => {return onChange('to', date, dateTo)}}
                showNavi="false"
                monthsNames=${t.monthsNames}
                daysNames=${t.daysNames}
              />
  ...
  static initComponent(element, config, par) {
    ...
    // }
    const data = DataRange.getOrCreateInstance(this)
  ...
  static jQueryInterface(config, par) {
    return this.each(function () {
      DataRange.initComponent(this, config, par);
    })
  }
...
EventHandler.on(window, EVENT_LOAD_DATA_API, () => {
  ...
  Array.from(document.querySelectorAll(SELECTOR_COMPONENT)).forEach(element => {
    DataRange.initComponent(element, Manipulator.getDataAttributes(element))
  })
})
 ...
 * jQuery
 * ------------------------------------------------------------------------
 * add .data-range to jQuery only if jQuery is present
 */

defineJQueryPlugin(DataRange)
